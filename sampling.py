...
class FluxSamplerParams:
    ...
    def INPUT_TYPES(s):
        return {"required": {
                ...
                "optional": {
                    "loras": ("LORA_PARAMS",),
                    "sigmas": ("SIGMAS",), # Setting this overrides steps, scheduler, denoise
    ...
    def execute(self, model, conditioning, latent_image, seed, sampler, scheduler, steps, guidance, max_shift, base_shift, denoise, loras=None, sigmas=None, custom_sampler = None):
        import random
        import time
                    ...
                    for ms in max_shift:
                        for bs in base_shift:
                            ...
                            for g in guidance:
                                ...
                                for s in sampler:
                                    # Adding support for custom_samplers
                                    if(custom_sampler == None):
                                        print(f"s={s}")
                                        samplerobj = comfy.samplers.sampler_object(s)
                                    else:
                                        print(f"custom_samplers={custom_sampler}")
                                        # samplerobj = comfy.samplers.sampler_object(custom_sampler)
                                        samplerobj = custom_sampler

                                    for sc in scheduler:
                                        for st in steps:
                                            for d in denoise:
                                                # Original code for sigmas: work_model, scheduler, steps, denoise
                                                if(sigmas == None):
